...
angular.module('aifxApp').controller('appController', function($scope, $ionicSideMenuDelegate, $rootScope, utils) {
    ...
    $scope.config = {
        ...
        urls: {
            ...
            candle: 'http://api-sandbox.oanda.com/v1/candles?instrument={{cross1}}_{{cross2}}&count={{count}}&candleFormat=midpoint&granularity={{period}}',
            ...
            /*
            select * from yahoo.finance.quoteslist where symbol in ('^GDAXI','^FTSE','^AORD','^N225','^GSPTSE')
             */
        ...
        yqls: {
            indexes: 'select * from yahoo.finance.quoteslist where symbol in ({{sets}})',
