...
for s in $(cat app/data/quandlSymbols.lst);
	do 
		...
		symbol=(${s//./ })
		quandlUrl="https://quandl.com/api/v3/datasets/${symbol[0]}/${symbol[1]}.csv?column=${symbol[2]}&collapse=daily&trim_start=2005-01-01&auth_token=pWGUEdRoPxqEdp66WRYv"

		echo $quandlUrl
		curl -L -G -A "Mozilla/5.0 (Windows NT 6.1; rv:19.0) Gecko/20100101 Firefox/19.0" -o $out $quandlUrl
